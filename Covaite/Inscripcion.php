	...
	if ($_POST['Guardar']) {
		...
		if ($Documento!="" && $Nombres!="" && $Apellidos!="" && $Genero!="" && $Telefono!="" && $Email!="" && $Labora!="" && $Formacion!="") {
				...
				$NombreC=mysqli_fetch_assoc($Ncon);
			        ...
			        <body>
			          <p style="font-size:28px; color:#333"><strong>Bienvenido: </strong> '.$Nombres.' '.$Apellidos.', Su registro ha sido exitoso, ahora pudes ingresar a la plataforma de '.$NombreC[Nombre].'.</p>
			          <br>
			          <div>
